...
const EnjoyLifeEnglish = () => {
    const [enjoyLifeEnglish, setEnjoyLifeEnglish] = useState(0)
    ...
    useEffect(() =>{
        ...
        const query = `*[_type == "book" && title == "Enjoy Life Forever" && language == "English" && date >= "${startOfYear}" && date <= "${endOfYear}"]{quantity}`;
  ...
  return (
    <div>{enjoyLifeEnglish}</div>
  )
}

export default EnjoyLifeEnglish