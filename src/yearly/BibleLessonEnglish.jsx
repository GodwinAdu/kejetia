...
const BibleLessonEnglish = () => {
    const [bibleLessonEnglish, setBibleLessonEnglish] = useState(0)
    ...
    useEffect(() =>{
       ...
       const query = `*[_type == "brochure" && title == "My Bible Lessons" && language == "English" && date >= "${startOfYear}" && date <= "${endOfYear}"]{quantity}`;
    ...
    client.fetch(query).then((datas) => {
        ...
        const sum = value?.reduce((result,item) => {
            ...
            setBibleLessonEnglish((prev)=> prev + sum)
        })
    },[])
  return (
    <>
      <div>{bibleLessonEnglish}</div>
    </>
  )
}

export default BibleLessonEnglish