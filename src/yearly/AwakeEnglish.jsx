...
const AwakeEnglish = () => {
    const [awakeEnglish, setAwakeEnglish] = useState(0)
    ...
    useEffect(() =>{
      ...
      const query = `*[_type == "magazine" && title == "Awake" && language == "English" && date >= "${startOfYear}" && date <= "${endOfYear}"]{quantity}`;
    ...
    client.fetch(query).then((datas) => {
        ...
        const sum = value?.reduce((result,item) => {
            ...
            setAwakeEnglish((prev)=> prev + sum)
        })
    },[])
  return (
    <>
      <div>{awakeEnglish}</div>
    </>
  )
}

export default AwakeEnglish